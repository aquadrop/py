    ...
    def reply(self, msg):
        ...
        if line == 'clear':
            ...
            with tf.Session() as session:
                ...
                preds = self.graph.get_tensor_by_name(
                    'top_predict_proba_op:0')
                ...
                outputs = session.run(preds, feed_dict={
                                  qp: qp_vc, ql: ql_vc, ip: ip_vc, il: il_vc,dp:self.config.dropout})


            print('outputs:', outputs)
            indices = outputs.indices.tolist()[0]
            values = outputs.values.tolist()[0]
...
class DmnInfer:
    ...
    def get_session(self):
        ...
        saver = tf.train.import_meta_graph(
            config.ckpt_path + 'dmn.weights.meta')
