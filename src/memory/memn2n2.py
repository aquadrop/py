...
class MemN2NDialog(object):
    ...
    def __init__(self, batch_size, vocab_size, candidates_size, sentence_size, embedding_size,
        ...
        Args:
            ...
            embeddings: The embedding matrix contains all words show up in data set.
    ...
    def _build_vars(self):
        with tf.variable_scope(self._name):
            self.embeddings = tf.Variable(self._embeddings, name='embeddings')
        ...
        self._nil_vars = set([self.embeddings.name, self.W.name])

    def _inference(self, stories, queries):
        with tf.variable_scope(self._name):
            q_emb = tf.nn.embedding_lookup(self.embeddings, queries)
            u_0 = tf.reduce_sum(q_emb, 1)
            u = [u_0]
            for _ in range(self._hops):
                m_emb = tf.nn.embedding_lookup(self.embeddings, stories)
